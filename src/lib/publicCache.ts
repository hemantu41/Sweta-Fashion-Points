 ...
 */
export async function publicCacheGet<T>(key: string, ttlSeconds = 300): Promise<T | null> {
  const l1 = memGet<T>(key);
 ...
 * Write to both tiers.
 */
export async function publicCacheSet(key: string, data: unknown, ttlSeconds = 300): Promise<void> {
  memSet(key, data, ttlSeconds);
