...
/**
 * Notify the seller (by email) when an admin changes the ticket status or adds a reply.
 * Called from the admin PATCH and admin comments POST routes.
 */
export async function notifySellerTicketUpdate(opts: {
  ...
  try {
    ...
    if (!seller?.email) {
      console.warn(`[sellerNotify] No email for seller ${sellerId} — ticket notification skipped`);
      return;
    }
    ...
    const body = `
      <p style="color:${C.text};font-size:15px;margin:0 0 16px;">
        Hello <strong>${seller.businessName}</strong>,
      </p>
      <table cellpadding="0" cellspacing="0" style="width:100%;margin-bottom:20px;">
        <tr>
      ...
      <p style="font-size:12px;color:${C.muted};text-align:center;margin-top:16px;">
        You can reply directly from your seller portal. Internal notes are not visible to you.
      </p>`;
    ...
    console.error('[sellerNotify] notifySellerTicketUpdate error:', err?.message);
  }
}
