...
export async function GET(request: NextRequest) {
  try {
    ...
    const stats = await adminCacheFirst<DashboardStats>('stats', async () => {
      ...
      const [
        totalOrdersRes, todayOrdersRes, productsRes,
        activeProductsRes, pendingRes, sellersRes,
        activeSellersRes, customersRes, earningsRes, todayEarningsRes,
      ] = await Promise.all([
        supabaseAdmin.from('spf_payment_orders').select('id', { count: 'exact', head: true }),
        supabaseAdmin.from('spf_payment_orders').select('id', { count: 'exact', head: true }).gte('created_at', todayStart),
        ...
        supabaseAdmin.from('spf_seller_earnings').select('seller_earning').limit(500),
      ...
      return {
        ...
        avgOrderValue,
        returnRate: 0, // computed from orders if needed
      };
    });
