...
// POST /api/auth/verify-signup-otp - Verify OTP for user signup email/mobile verification
export async function POST(request: NextRequest) {
  try {
    const body = await request.json();
    const { type, value, otp } = body; // type: 'email' or 'mobile', value: email or mobile number
