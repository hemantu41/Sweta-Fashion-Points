...
const HERO_IMAGES: Record<string, string> = {
  default: 'https://images.unsplash.com/photo-1483985988355-763728e1935b?w=1800&h=800&fit=crop&crop=center&q=95',
  daily: 'https://images.unsplash.com/photo-1509631179647-0177331693ae?w=1800&h=800&fit=crop&crop=top&q=95',
  party: 'https://images.unsplash.com/photo-1515886657613-9f3515b0c78f?w=1800&h=800&fit=crop&crop=top&q=95',
  ethnic: 'https://images.unsplash.com/photo-1610030469983-98e550d6193c?w=1800&h=800&fit=crop&crop=top&q=95',
  seasonal: 'https://images.unsplash.com/photo-1485968579580-b6d095142e6e?w=1800&h=800&fit=crop&crop=top&q=95',
};
...
export default function WomensPage() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(
        `/api/products?category=womens${user?.latitude && user?.longitude ? `&userLat=${user.latitude}&userLng=${user.longitude}` : ''}`,
        { cache: 'no-store' }
      );
  ...
  const fabrics = ['Cotton', 'Silk', 'Chiffon', 'Georgette', 'Crepe', 'Rayon'];
  const occasions = ['Casual', 'Formal', 'Party', 'Ethnic', 'Bridal'];
  const colors = [
    { name: 'Black', hex: '#1A1A1A' },
    { name: 'White', hex: '#F0EDE8' },
    { name: 'Rose', hex: '#E8919A' },
    { name: 'Blush', hex: '#F4C0C0' },
    { name: 'Navy', hex: '#2C3E6B' },
    { name: 'Lavender', hex: '#B094C8' },
    { name: 'Sage', hex: '#7A9E7A' },
  ...
  const categories = [
    ...
    { id: 'ethnic', en: 'Ethnic Wear', hi: 'एथनिक वियर' },
    { id: 'seasonal', en: 'Seasonal', hi: 'सीज़नल' },
  ];
  ...
  const getCategoryTitle = () => {
    const map: Record<string, { en: string; hi: string }> = {
      daily: { en: "Women's Daily Wear", hi: 'महिलाओं का डेली वियर' },
      party: { en: "Women's Party Wear", hi: 'महिलाओं का पार्टी वियर' },
      ethnic: { en: "Women's Ethnic Wear", hi: 'महिलाओं का एथनिक वियर' },
      seasonal: { en: "Women's Seasonal", hi: 'महिलाओं का सीज़नल' },
    };
    if (activeCategory && map[activeCategory]) {
      return language === 'hi' ? map[activeCategory].hi : map[activeCategory].en;
    }
    return language === 'hi' ? "महिलाओं का कलेक्शन" : "Women's Collection";
  };

  const getCategorySubtitle = () => {
    const map: Record<string, { en: string; hi: string }> = {
      daily: { en: 'Comfort & style for every day', hi: 'हर दिन के लिए आरामदायक स्टाइल' },
      party: { en: 'Shine bright at every occasion', hi: 'हर अवसर पर चमकें' },
      ethnic: { en: 'Grace rooted in tradition', hi: 'परंपरा में बसी सुंदरता' },
      seasonal: { en: 'Dressed for every season', hi: 'हर मौसम के लिए तैयार' },
    };
    if (activeCategory && map[activeCategory]) {
      return language === 'hi' ? map[activeCategory].hi : map[activeCategory].en;
    }
    return language === 'hi' ? 'प्रीमियम फैशन, हर अवसर के लिए' : 'Premium fashion for every moment';
  };

  const activeFiltersCount =
    (priceRange[0] > 0 || priceRange[1] < 100000 ? 1 : 0) +
    selectedSizes.length + selectedFabrics.length + selectedColors.length + selectedOccasions.length;
        ...
        <div className="grid grid-cols-3 gap-1.5">
          {sizes.map((size) => (
            <button key={size}
              ...
              className={`py-1.5 rounded-lg text-[11px] font-medium border transition-all ${selectedSizes.includes(size) ? 'bg-[#1A1A1A] text-white border-[#1A1A1A] shadow-sm' : 'bg-white text-[#6B6B6B] border-[#E0DBD4] hover:border-[#1A1A1A] hover:text-[#1A1A1A]'}`}>
              {size}
            </button>
          ))}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {colors.map((color) => (
            <button key={color.name}
              ...
              title={color.name}
              className={`w-6 h-6 rounded-full transition-all ring-offset-2 ring-offset-white ${selectedColors.includes(color.name) ? 'ring-2 ring-[#722F37] scale-110' : 'ring-1 ring-transparent hover:ring-[#BCBCBC] hover:scale-105'} ${color.hex === '#F0EDE8' || color.hex === '#F5F0E8' ? 'border border-[#D4D0CB]' : ''}`}
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F4]">
      ...
      <div className="relative w-full h-[360px] sm:h-[460px] md:h-[540px] overflow-hidden">
        ...
        />
        {/* Warm rose colour grade */}
        <div className="absolute inset-0 bg-rose-950/15" />
        <div className="absolute inset-0 bg-gradient-to-r from-black/68 via-black/38 to-black/10" />
        <div className="absolute inset-0 flex flex-col justify-end pb-16 px-12 sm:px-16 lg:px-24">
          <nav className="flex items-center gap-2 text-[11px] text-white/70 mb-4 tracking-wide" style={{ textShadow: '0 1px 6px rgba(0,0,0,0.8)' }}>
            ...
            <span>/</span>
            <Link href="/womens" className="hover:text-white transition-colors">Women's</Link>
            {activeCategory && (
              <>
                <span>/</span>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8" onClick={() => setQuickViewProduct(null)}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col sm:flex-row" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 p-7 sm:p-9 overflow-y-auto flex flex-col">
              <p className="text-[9.5px] tracking-[0.25em] uppercase text-[#ADADAD] font-semibold mb-2">Fashion Points · Women's</p>
              ...
              {quickViewProduct.sizes && quickViewProduct.sizes.length > 0 && (
                <div className="mb-6">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#ADADAD] mb-3">Select Size</p>
                  <div className="flex flex-wrap gap-2">
