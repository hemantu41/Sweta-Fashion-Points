...
const HERO_IMAGES: Record<string, string> = {
  default: 'https://images.unsplash.com/photo-1542291026-7eec264c27ff?w=1800&h=800&fit=crop&crop=center&q=95',
  'sport-shoes': 'https://images.unsplash.com/photo-1515955656352-a1fa3ffcd111?w=1800&h=800&fit=crop&crop=center&q=95',
  sneakers: 'https://images.unsplash.com/photo-1460353581641-37baddab0fa2?w=1800&h=800&fit=crop&crop=center&q=95',
  'formal-shoes': 'https://images.unsplash.com/photo-1614252235316-8c857d38b5f4?w=1800&h=800&fit=crop&crop=center&q=95',
  slippers: 'https://images.unsplash.com/photo-1603487742131-4160ec999306?w=1800&h=800&fit=crop&crop=center&q=95',
};
...
export default function FootwearPage() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(
        `/api/products?category=footwear${user?.latitude && user?.longitude ? `&userLat=${user.latitude}&userLng=${user.longitude}` : ''}`,
        { cache: 'no-store' }
      );
  ...
  const shoeTypes = ['Casual', 'Formal', 'Sports', 'Party', 'Ethnic', 'Outdoor'];
  const materials = ['Leather', 'Canvas', 'Mesh', 'Synthetic', 'Suede', 'Rubber'];
  const colors = [
  ...
  const categories = [
    { id: null, en: 'All Footwear', hi: 'सभी फुटवियर' },
    { id: 'sport-shoes', en: 'Sport Shoes', hi: 'स्पोर्ट शूज़' },
    { id: 'sneakers', en: 'Sneakers', hi: 'स्नीकर्स' },
    { id: 'formal-shoes', en: 'Formal Shoes', hi: 'फॉर्मल शूज़' },
    { id: 'slippers', en: 'Slippers', hi: 'चप्पल / सैंडल' },
  ];
  ...
  const filteredProducts = allProducts
    .filter((product) => {
      ...
      if (selectedMaterials.length > 0 && product.fabric) {
        if (!selectedMaterials.includes(product.fabric)) return false;
      }
      if (selectedColors.length > 0 && product.color) {
        if (!selectedColors.includes(product.color)) return false;
      }
      if (selectedTypes.length > 0 && product.occasion) {
        if (!selectedTypes.includes(product.occasion)) return false;
      }
      return true;
    })
    .sort((a, b) => {
  ...
  const getCategoryTitle = () => {
    const map: Record<string, { en: string; hi: string }> = {
      'sport-shoes': { en: 'Sport Shoes', hi: 'स्पोर्ट शूज़' },
      sneakers: { en: 'Sneakers', hi: 'स्नीकर्स' },
      'formal-shoes': { en: 'Formal Shoes', hi: 'फॉर्मल शूज़' },
      slippers: { en: 'Slippers & Sandals', hi: 'चप्पल व सैंडल' },
    };
    if (activeCategory && map[activeCategory]) {
      return language === 'hi' ? map[activeCategory].hi : map[activeCategory].en;
    }
    return language === 'hi' ? 'फुटवियर कलेक्शन' : 'Footwear Collection';
  };

  const getCategorySubtitle = () => {
    const map: Record<string, { en: string; hi: string }> = {
      'sport-shoes': { en: 'Built for performance, made for style', hi: 'परफॉर्मेंस और स्टाइल का कॉम्बो' },
      sneakers: { en: 'Step out in style every single day', hi: 'हर दिन के लिए स्टाइलिश स्नीकर्स' },
      'formal-shoes': { en: 'Crafted for the occasions that matter', hi: 'खास मौकों के लिए शानदार फॉर्मल शूज़' },
      slippers: { en: 'Comfort from the first step', hi: 'आरामदायक चप्पल व सैंडल' },
    };
    if (activeCategory && map[activeCategory]) {
      return language === 'hi' ? map[activeCategory].hi : map[activeCategory].en;
    }
    return language === 'hi' ? 'हर कदम के साथ स्टाइल' : 'Every step, in style';
  };

  const activeFiltersCount =
    (priceRange[0] > 0 || priceRange[1] < 100000 ? 1 : 0) +
    selectedSizes.length + selectedMaterials.length + selectedColors.length + selectedTypes.length;
  ...
  const FilterContent = () => (
    <>
      ...
      <div className="mb-5 pb-5 border-b border-[#F0EDE8]">
        <h3 className="text-[9px] font-bold text-[#ADADAD] uppercase tracking-[0.25em] mb-3">
          {language === 'hi' ? 'साइज़ (UK)' : 'Size (UK)'}
        </h3>
        <div className="grid grid-cols-4 gap-1.5">
          {shoeSizes.map((size) => (
            <button key={size}
              onClick={() => setSelectedSizes(prev => prev.includes(size) ? prev.filter(s => s !== size) : [...prev, size])}
              className={`py-1.5 rounded-lg text-[10.5px] font-medium border transition-all ${selectedSizes.includes(size) ? 'bg-[#1A1A1A] text-white border-[#1A1A1A] shadow-sm' : 'bg-white text-[#6B6B6B] border-[#E0DBD4] hover:border-[#1A1A1A] hover:text-[#1A1A1A]'}`}>
              {size.replace('UK ', '')}
            </button>
          ))}
        </div>
      ...
      <div className="mb-5 pb-5 border-b border-[#F0EDE8]">
        ...
        <div className="space-y-2">
          {shoeTypes.map((type) => {
            ...
            return (
              <label key={type} onClick={() => setSelectedTypes(prev => prev.includes(type) ? prev.filter(t => t !== type) : [...prev, type])} className="flex items-center gap-2.5 cursor-pointer group">
                ...
                <span className={`text-[11.5px] transition-colors ${checked ? 'text-[#1A1A1A] font-medium' : 'text-[#8A8A8A] group-hover:text-[#3A3A3A]'}`}>{type}</span>
              </label>
            );
          })}
        </div>
      ...
      <div className="mb-5 pb-5 border-b border-[#F0EDE8]">
        <h3 className="text-[9px] font-bold text-[#ADADAD] uppercase tracking-[0.25em] mb-3">
          {language === 'hi' ? 'मटेरियल' : 'Material'}
        </h3>
        <div className="space-y-2">
          {materials.map((mat) => {
            const checked = selectedMaterials.includes(mat);
            return (
              <label key={mat} onClick={() => setSelectedMaterials(prev => prev.includes(mat) ? prev.filter(m => m !== mat) : [...prev, mat])} className="flex items-center gap-2.5 cursor-pointer group">
                <span className={`w-3.5 h-3.5 rounded border-2 flex-shrink-0 flex items-center justify-center transition-all ${checked ? 'border-[#722F37] bg-[#722F37]' : 'border-[#D0C9C0] group-hover:border-[#722F37]'}`}>
                  {checked && <svg className="w-2 h-2 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24" strokeWidth={3}><path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" /></svg>}
                </span>
                <span className={`text-[11.5px] transition-colors ${checked ? 'text-[#1A1A1A] font-medium' : 'text-[#8A8A8A] group-hover:text-[#3A3A3A]'}`}>{mat}</span>
              </label>
            );
          })}
        </div>
        ...
        <div className="flex flex-wrap gap-2">
          {colors.map((color) => (
            <button key={color.name}
              ...
              title={color.name}
              className={`w-6 h-6 rounded-full transition-all ring-offset-2 ring-offset-white ${selectedColors.includes(color.name) ? 'ring-2 ring-[#722F37] scale-110' : 'ring-1 ring-transparent hover:ring-[#BCBCBC] hover:scale-105'} ${color.hex === '#F0EDE8' || color.hex === '#D4C4A8' ? 'border border-[#D4D0CB]' : ''}`}
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F4]">
      ...
      <div className="relative w-full h-[360px] sm:h-[460px] md:h-[540px] overflow-hidden">
        ...
        />
        {/* Dark stone colour grade — works well with shoes */}
        <div className="absolute inset-0 bg-stone-950/18" />
        ...
        <div className="absolute inset-0 flex flex-col justify-end pb-16 px-12 sm:px-16 lg:px-24">
          <nav className="flex items-center gap-2 text-[11px] text-white/70 mb-4 tracking-wide" style={{ textShadow: '0 1px 6px rgba(0,0,0,0.8)' }}>
            ...
            <span>/</span>
            <Link href="/footwear" className="hover:text-white transition-colors">Footwear</Link>
            {activeCategory && (
              <>
                <span>/</span>
                <span className="text-white/90 capitalize">{activeCategory.replace('-', ' ')}</span>
              </>
            )}
          </nav>
          <h1
          ...
          <div className="flex-1 min-w-0">
            ...
            {loading ? (
              ...
              <>
                ...
                {hasMore && (
                  <div className="mt-14 text-center">
                    ...
                    <p className="mt-3 text-[11px] text-[#ADADAD]">{filteredProducts.length - visibleCount} {language === 'hi' ? 'और उत्पाद' : 'more products'}</p>
                  </div>
                )}
              </>
            ) : (
              <div className="py-24 text-center">
                ...
                <p className="text-[14px] text-[#6B6B6B] mb-2">{language === 'hi' ? 'कोई उत्पाद नहीं मिला' : 'No products found'}</p>
      ...
      {quickViewProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8" onClick={() => setQuickViewProduct(null)}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col sm:flex-row" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="relative w-full sm:w-[340px] aspect-square sm:aspect-auto flex-shrink-0 bg-[#F0EDE8]">
            ...
            <div className="flex-1 p-7 sm:p-9 overflow-y-auto flex flex-col">
              <p className="text-[9.5px] tracking-[0.25em] uppercase text-[#ADADAD] font-semibold mb-2">Fashion Points · Footwear</p>
              ...
              {quickViewProduct.sizes && quickViewProduct.sizes.length > 0 && (
                <div className="mb-6">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#ADADAD] mb-3">Select Size (UK)</p>
                  <div className="flex flex-wrap gap-2">
