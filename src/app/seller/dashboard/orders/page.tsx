...
const STATUS_CONFIG: Record<string, { label: string; bg: string; color: string }> = {
  new: { label: 'New', bg: '#FEF7EA', color: '#C49A3C' },
  captured: { label: 'New', bg: '#FEF7EA', color: '#C49A3C' },
  packed: { label: 'Packed', bg: '#EBF2FB', color: '#1565C0' },
  shipped: { label: 'Shipped', bg: '#F3EBF7', color: '#6B1A8B' },
  delivered: { label: 'Delivered', bg: '#EBF7EF', color: '#2E7D32' },
  returned: { label: 'Returned', bg: '#FDF3F3', color: '#C62828' },
            ...
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-gray-50">
                {filtered.map(order => {
                  ...
                  return (
                    <tr key={order.id} className="hover:bg-[#C49A3C]/5 transition-colors">
                      ...
                      <td className="px-4 py-3 text-center">
                        ...
                        {(s === 'packed' || s === 'shipped') && (
                          <button className="px-3 py-1 text-xs font-semibold border border-[#1565C0] text-[#1565C0] rounded-lg hover:bg-blue-50">Track</button>
                        )}
                        {s === 'delivered' && (
                        ...
                        {s === 'returned' && (
                          <button className="px-3 py-1 text-xs font-semibold border border-[#C62828] text-[#C62828] rounded-lg hover:bg-red-50">Review</button>
                        )}
                      </td>
                    </tr>
                  );
