...
/* ─── Category Types ────────────────────────────────────────────────────────── */
interface CatOption { id: string; name: string; nameHi: string; icon: string; }

/* ─── Occasion tags ─────────────────────────────────────────────────────────── */
const OCCASION_TAGS = [
  { id: 'wedding',        label: 'Wedding Season' },
...
const COLORS_LIST = [
  ...
  { name: 'Plum',         hex: '#7E22CE' }, { name: 'White',        hex: '#F9FAFB' },
  { name: 'Off White',    hex: '#FAF9F6' }, { name: 'Ivory',        hex: '#FFFFF0' },
  { name: 'Cream',        hex: '#FFFBEB' }, { name: 'Beige',        hex: '#D2B48C' },
  { name: 'Khaki',        hex: '#C3B091' }, { name: 'Caramel',      hex: '#C19A6B' },
  { name: 'Brown',        hex: '#92400E' }, { name: 'Chocolate',    hex: '#78350F' },
  { name: 'Silver',       hex: '#9CA3AF' }, { name: 'Grey',         hex: '#6B7280' },
...
export default function EditProductPage() {
  ...
  /* Pending category IDs from product fetch (applied once cats are loaded) */
  const pendingCats = useRef<{ l1: string; l2: string; l3: string } | null>(null);
  const catsApplied = useRef(false);
  ...
  /* ── Apply pending category IDs once L1 cats are loaded ── */
  useEffect(() => {
    if (!l1Cats.length || !pendingCats.current || catsApplied.current) return;
    const { l1: pl1, l2: pl2, l3: pl3 } = pendingCats.current;
    if (pl1) {
      setL1(pl1);
      // l2 and l3 will be applied after their cats load
    }
    catsApplied.current = true;
  }, [l1Cats]);
  ...
  useEffect(() => {
    ...
    fetch(`/api/categories?parent_id=${l1}&active=true`)
      ...
      .then(d => {
        if (d.success) {
          const cats = (d.data || []).map((c: any) => ({ id: c.id, name: c.name, nameHi: c.name_hindi || '', icon: c.icon || '' }));
          setL2Cats(cats);
          // Apply pending l2 if available
          if (pendingCats.current?.l2) {
            setL2(pendingCats.current.l2);
          }
        }
      })
      .catch(() => {});
    ...
    fetch(`/api/categories?parent_id=${l2}&active=true`)
      ...
      .then(d => {
        if (d.success) {
          const cats = (d.data || []).map((c: any) => ({ id: c.id, name: c.name, nameHi: c.name_hindi || '', icon: c.icon || '' }));
          setL3Cats(cats);
          // Apply pending l3 if available
          if (pendingCats.current?.l3) {
            setL3(pendingCats.current.l3);
          }
        }
      })
      .catch(() => {});
  ...
  useEffect(() => {
    ...
    const doFetch = async () => {
      try {
        // Wait briefly for sellerId to be populated via the seller profile fetch
        ...
        const res = await fetch(`/api/products/${productUUID}?sellerId=${sId || 'pending'}`);
        ...
        setExistingProductId(p.productId || '');
        setProductSku(p.productId || '');
        setName(p.name || '');
        setNameHi(p.nameHi || '');
        ...
        setMrp(p.originalPrice?.toString() || '');
        setExistingPrice(p.price || 0);

        // Colors: stored as [{name, hex}] — extract names
        if (Array.isArray(p.colors)) {
          setColors(p.colors.map((c: any) => (typeof c === 'string' ? c : c?.name)).filter(Boolean));
        }

        // Store category IDs to apply once cats load
        pendingCats.current = {
          l1: p.l1CategoryId || '',
          l2: p.l2CategoryId || '',
          l3: p.l3CategoryId || '',
        ...
        // If L1 cats are already loaded, apply immediately
        setL1Cats(prev => {
          if (prev.length && pendingCats.current && !catsApplied.current) {
            const { l1: pl1 } = pendingCats.current;
            if (pl1) setL1(pl1);
            catsApplied.current = true;
          }
          return prev;
  ...
  function selectL1(id: string) { setL1(id); setL2(''); setL3(''); setL2Cats([]); setL3Cats([]); setCatSearch(''); setSearchResults([]); pendingCats.current = null; }
  function selectL2(id: string) { setL2(id); setL3(''); setL3Cats([]); }
  function selectL3(id: string) { setL3(id); }
  function resetCat() { setL1(''); setL2(''); setL3(''); setL2Cats([]); setL3Cats([]); pendingCats.current = null; }
  ...
  const isValid = name && l1 && description && price && stock && checked1 && checked2;
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const res = await fetch(`/api/products/${productUUID}`, {
        method: 'PUT',
  ...
  if (loadingProduct) {
    return (
      <div className="flex flex-col items-center justify-center py-20 gap-4" style={{ fontFamily: 'var(--font-dm-sans,DM Sans,sans-serif)' }}>
  ...
  if (submitted) {
    return (
      <div className="max-w-lg mx-auto text-center py-16" style={{ fontFamily: 'var(--font-dm-sans,DM Sans,sans-serif)' }}>
        ...
        <p className="text-sm text-[#666] mb-1">Your changes have been sent to admin for approval.</p>
        <p className="text-sm text-[#999] mb-6">The product will go live again once approved — usually within 24 hours.</p>
  ...
  return (
    <form onSubmit={handleSubmit} style={{ fontFamily: 'var(--font-dm-sans,DM Sans,sans-serif)' }}>
      ...
      <div className="flex items-center justify-between mb-6">
        <div>
          <h1 className="text-xl font-bold text-[#5B1A3A]" style={{ fontFamily: 'var(--font-playfair)' }}>Edit Product</h1>
          {existingProductId && <p className="text-xs text-[#999] mt-0.5">ID: {existingProductId}</p>}
        </div>
        <button type="button" onClick={() => router.push('/seller/dashboard/products')}
          className="text-xs font-medium text-[#5B1A3A] border border-[#E8E0E4] px-3 py-1.5 rounded-lg hover:bg-[#F5EDF2] transition-colors">
          ← Back to Products
        </button>
      </div>

      {/* Edit notice */}
      <div className="mb-5 flex items-start gap-2.5 px-4 py-3 bg-amber-50 border border-amber-200 rounded-xl text-xs text-amber-800">
        <AlertCircle size={14} className="mt-0.5 flex-shrink-0 text-amber-500" />
        <span>Saving changes will send this product back for admin review. It will be hidden from customers until re-approved.</span>
      </div>
        ...
        <div className="lg:col-span-3 space-y-5">
            ...
            {l3 && l1Label && l2Label && l3Label ? (
              ...
              <>
                <div className="mb-1">
                  ...
                  {catsLoading ? (
                    <div className="flex gap-2">{[1,2,3,4,5].map(i => <div key={i} className="w-20 h-16 rounded-[14px] bg-[#E8E0E4] animate-pulse" />)}</div>
                  ) : l1Cats.length === 0 ? (
                ...
                {l2 && l3Cats.length > 0 && (
                  <div className="mt-4 animate-in fade-in slide-in-from-top-2 duration-200">
                    <p className="text-xs font-medium text-[#666] mb-2">Step 3: Product Type</p>
                    <div className="flex flex-wrap gap-2">
                      {l3Cats.map(c => (
          ...
          <SectionCard title="Product Details">
            <div className="space-y-4">
              <div>
                <FieldLabel>Product ID (auto-assigned, cannot be changed)</FieldLabel>
                <div className="px-3 py-2 text-sm bg-[#F5F0E8] border border-[#E8E0E4] rounded-lg text-[#5B1A3A] font-mono font-semibold">
                  {existingProductId || '—'}
                </div>
              </div>
              <div>
          ...
          <SectionCard title="Photos & Videos">
            ...
            <div
              ...
              className={`mb-3 border-2 border-dashed rounded-xl flex flex-col items-center justify-center gap-2 py-5 cursor-pointer transition-all ${
                isDragOver ? 'border-[#C49A3C] bg-[rgba(196,154,60,0.05)]'
                  : images.length >= 8 ? 'border-[#E8E0E4] cursor-not-allowed opacity-50'
            ...
            <p className="text-[10px] text-[#999] mt-1">{images.length}/8 photos {images.length < 3 ? `— ${3 - images.length} more recommended` : '— requirement met'}</p>
          </SectionCard>
          ...
          <SectionCard title="Pricing & Stock">
            {existingPrice > 0 && (
              <div className="mb-4 px-3 py-2 bg-blue-50 border border-blue-100 rounded-lg text-xs text-blue-700">
                Current customer price: <strong>₹{existingPrice.toLocaleString('en-IN')}</strong> — enter your new base price below to update.
              </div>
            )}
          ...
          <SectionCard title="Occasion Tags"
          ...
          <SectionCard title="Seller Details">
            <div className="space-y-4">
              <div>
                ...
                <input value={sellerName} onChange={e => setSellerName(e.target.value)} placeholder="Your shop / business name" className={INPUT_CLS} />
              </div>
              <div>
                <FieldLabel>Address</FieldLabel>
                <input value={sellerAddress} onChange={e => setSellerAddress(e.target.value)} placeholder="Shop / warehouse address" className={INPUT_CLS} />
              </div>
              <div>
                ...
                <input type="text" inputMode="numeric" value={sellerPincode}
                  ...
                  placeholder="6-digit pincode" maxLength={6} className={INPUT_CLS} />
              </div>
              <div>
                <FieldLabel>Phone Number</FieldLabel>
                <div className="flex gap-2">
                  <div className="flex items-center px-3 bg-[#F5F0E8] border border-[#E8E0E4] rounded-lg text-sm text-[#666] font-medium flex-shrink-0">+91</div>
                  <input type="tel" inputMode="numeric" value={sellerPhone}
                    onChange={e => setSellerPhone(e.target.value.replace(/\D/g, '').slice(0, 10))}
                    placeholder="10-digit phone number" maxLength={10} className={`${INPUT_CLS} flex-1`} />
                </div>
                <label className="flex items-start gap-2.5 mt-2.5 cursor-pointer">
                  <input type="checkbox" checked={sellerPhoneConsent} onChange={e => setSellerPhoneConsent(e.target.checked)} className="mt-0.5 accent-[#5B1A3A] flex-shrink-0" />
                  <span className="text-[11px] text-[#666] leading-relaxed">I confirm this is my registered business phone number and consent to receive order updates on this number.</span>
                </label>
              </div>
            </div>
          </SectionCard>
          ...
          <SectionCard title="Other Details">
            <div className="space-y-4">
              <div>
                ...
                <input value={brand} onChange={e => setBrand(e.target.value)} placeholder="Type your brand name (leave blank if unbranded)" className={INPUT_CLS} />
              </div>
          ...
          <div className="bg-white rounded-[14px] border border-[rgba(196,154,60,0.08)] shadow-[0_2px_16px_rgba(91,26,58,0.04)] p-5">
            ...
            <button type="submit" disabled={submitting || !isValid}
              ...
              style={{ background: 'linear-gradient(135deg,#5B1A3A,#7A2350)' }}>
              {submitting ? 'Submitting…' : 'Save Changes & Submit for Approval'}
            </button>
            <p className="text-[10px] text-[#999] text-center mt-2">Product will be hidden from customers until re-approved (usually within 24 hours)</p>
          </div>
        </div>
              ...
              <div className="p-4">
                <div className="aspect-[3/4] rounded-xl overflow-hidden mb-3 bg-[#F5EDF2] flex items-center justify-center">
                  {images[0] ? (
                    ...
                    <div className="text-center">
                      ...
                      <p className="text-[10px] text-[#999]">Product image</p>
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-baseline gap-2 mb-1">
                  {totalCustomerPrice > 0 ? (
                    ...
                    <span className="text-base font-bold text-[#5B1A3A]">₹{existingPrice.toLocaleString('en-IN')}</span>
                  ) : (
                ...
                <div className="mt-3 pt-3 border-t border-[rgba(196,154,60,0.08)]">
                  ...
                  <div className="h-1.5 bg-[#F5EDF2] rounded-full overflow-hidden">
                    <div className="h-full rounded-full transition-all" style={{
                      width: `${[l1, name, description, price, stock, images.length > 0, fabric].filter(Boolean).length / 7 * 100}%`,
                      background: 'linear-gradient(90deg,#5B1A3A,#C49A3C)'
                    }} />
                  </div>
                  <p className="text-[9px] text-[#999] mt-1">
                    {[l1, name, description, price, stock, images.length > 0, fabric].filter(Boolean).length}/7 fields complete
                  </p>
                </div>
              </div>
            </div>
