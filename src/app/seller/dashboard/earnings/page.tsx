...
const CYCLE_STATUS_CFG = {
  paid: { label: 'Paid', bg: '#EBF7EF', color: '#1A6B3A', icon: '✓' },
  processing: { label: 'Processing', bg: '#EBF2FB', color: '#1A3D6B', icon: '↻' },
  pending: { label: 'Upcoming', bg: '#FEF7EA', color: '#8B5E0A', icon: '⏳' },
  on_hold: { label: 'On Hold', bg: '#FDF3F3', color: '#8B1A1A', icon: '⚠' },
};
...
function CycleRow({ cycle, isFirst }: { cycle: SettlementCycle; isFirst: boolean }) {
  ...
  return (
    <div className={`rounded-xl border overflow-hidden ${isFirst ? 'border-[#8B1A1A]/20' : 'border-gray-100'}`}>
      <button
        onClick={() => setOpen(o => !o)}
        className="w-full flex items-center justify-between px-4 py-3 hover:bg-gray-50/50 text-left"
        style={isFirst ? { background: '#FDF8F8' } : {}}>
        ...
        <div className="flex items-center gap-3">
          <div className="text-right">
            <p className="text-sm font-bold" style={{ color: '#1A6B3A', fontFamily: 'var(--font-playfair)' }}>{fmt(cycle.netAmount)}</p>
      ...
      {open && (
        <div className="border-t border-gray-50 px-4 py-3 bg-gray-50/30">
          <div className="grid grid-cols-3 gap-3 mb-3">
            {[
              { label: 'Gross Revenue', value: fmt(cycle.grossAmount), color: '#1A6B3A' },
              { label: 'Platform Fee (10%)', value: `– ${fmt(cycle.commissionAmount)}`, color: '#8B1A1A' },
              { label: 'Net Payout', value: fmt(cycle.netAmount), color: '#1A3D6B' },
            ].map(item => (
  ...
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-5" style={{ fontFamily: 'var(--font-dm-sans, DM Sans, sans-serif)' }}>
      ...
      <div className="space-y-4">
        ...
        {[
          { label: 'Net Paid Out', value: summary.paid, bg: 'bg-white border border-gray-100', tc: '#1A6B3A' },
          { label: 'Pending Payout', value: summary.pending, bg: 'bg-amber-50 border border-amber-100', tc: '#8B5E0A', note: `Next: ${formatDate(nextPayout)}` },
        ...
        <div className="rounded-xl p-4 border border-gray-100 bg-white">
          ...
          <div className="mt-2 h-1.5 rounded-full bg-gray-100 overflow-hidden">
            {(() => {
              ...
              return <div className="h-full rounded-full" style={{ width: `${pct}%`, background: '#8B1A1A' }} />;
            })()}
          </div>
      ...
      <div className="lg:col-span-2 space-y-4">
        ...
        <div className="flex items-center gap-2">
          {(['settlements', 'transactions'] as const).map(v => (
            <button key={v} onClick={() => setView(v)}
              ...
              style={view === v ? { background: '#8B1A1A' } : {}}>
        ...
        {loading ? (
          ...
          <div className="space-y-3">
            {cycles.length === 0 ? (
              <div className="bg-white rounded-xl border border-gray-100 shadow-sm flex flex-col items-center py-12 text-gray-400 gap-2">
                  ...
                  {filtered.map(e => {
                    ...
                    return (
                      <tr key={e.id} className="hover:bg-gray-50/50">
                        ...
                        <td className="px-4 py-3 text-right font-semibold text-xs" style={{ color: '#1A6B3A', fontFamily: 'var(--font-playfair)' }}>
