...
const HERO_IMAGES: Record<string, string> = {
  default: 'https://images.unsplash.com/photo-1471286174890-9c112ac6476a?w=1800&h=800&fit=crop&crop=center&q=95',
  '0-3': 'https://images.unsplash.com/photo-1519689680058-324335c77eba?w=1800&h=800&fit=crop&crop=center&q=95',
  '4-7': 'https://images.unsplash.com/photo-1535572290543-960a8046f5af?w=1800&h=800&fit=crop&crop=center&q=95',
  '8-12': 'https://images.unsplash.com/photo-1490730141103-6cac27aaab94?w=1800&h=800&fit=crop&crop=top&q=95',
};
...
export default function KidsPage() {
  ...
  const fetchProducts = async () => {
    try {
      const response = await fetch(
        `/api/products?category=kids${user?.latitude && user?.longitude ? `&userLat=${user.latitude}&userLng=${user.longitude}` : ''}`,
        { cache: 'no-store' }
      );
  ...
  const ageSizes = ['0-3M', '3-6M', '6-12M', '1-2Y', '2-3Y', '3-4Y', '4-5Y', '5-6Y', '6-7Y', '7-8Y', '8-9Y', '9-10Y'];
  const genders = ['Boys', 'Girls', 'Unisex'];
  const fabrics = ['Cotton', 'Soft Cotton', 'Fleece', 'Denim', 'Polyester'];
  const colors = [
    { name: 'Red', hex: '#CC2222' },
  ...
  const priceRanges = [
    ...
    { label: 'Above ₹3,000', min: 3000, max: 100000 },
  ];
  const categories = [
    { id: null, en: 'All Kids', hi: 'सभी' },
    { id: '0-3', en: '0–3 Years', hi: '0-3 साल' },
    { id: '4-7', en: '4–7 Years', hi: '4-7 साल' },
    { id: '8-12', en: '8–12 Years', hi: '8-12 साल' },
  ];
  ...
  const getCategoryTitle = () => {
    const map: Record<string, { en: string; hi: string }> = {
      '0-3': { en: 'Kids 0–3 Years', hi: '0-3 साल के बच्चे' },
      '4-7': { en: 'Kids 4–7 Years', hi: '4-7 साल के बच्चे' },
      '8-12': { en: 'Kids 8–12 Years', hi: '8-12 साल के बच्चे' },
    };
    if (activeCategory && map[activeCategory]) {
      return language === 'hi' ? map[activeCategory].hi : map[activeCategory].en;
    }
    return language === 'hi' ? "बच्चों का कलेक्शन" : "Kids' Collection";
  };

  const getCategorySubtitle = () => {
    const map: Record<string, { en: string; hi: string }> = {
      '0-3': { en: 'Soft, safe and adorable for little ones', hi: 'नन्हे बच्चों के लिए कोमल और सुंदर कपड़े' },
      '4-7': { en: 'Fun styles for active little adventurers', hi: 'सक्रिय बच्चों के लिए मजेदार स्टाइल' },
      '8-12': { en: 'Cool and comfortable for growing kids', hi: 'बड़े बच्चों के लिए स्टाइलिश कपड़े' },
    };
    if (activeCategory && map[activeCategory]) {
      return language === 'hi' ? map[activeCategory].hi : map[activeCategory].en;
    }
    return language === 'hi' ? 'बच्चों के लिए प्यारे और आरामदायक कपड़े' : 'Comfortable, playful and stylish for every child';
  };

  const activeFiltersCount =
    (priceRange[0] > 0 || priceRange[1] < 100000 ? 1 : 0) +
    selectedSizes.length + selectedFabrics.length + selectedColors.length + selectedGenders.length;
  ...
  const FilterContent = () => (
    <>
        ...
        <div className="flex gap-2">
          {genders.map((g) => (
            <button key={g}
              ...
              className={`flex-1 py-1.5 rounded-lg text-[11px] font-medium border transition-all ${selectedGenders.includes(g) ? 'bg-[#1A1A1A] text-white border-[#1A1A1A]' : 'bg-white text-[#6B6B6B] border-[#E0DBD4] hover:border-[#1A1A1A] hover:text-[#1A1A1A]'}`}>
              {g}
            </button>
          ))}
        </div>
      ...
      <div className="mb-5 pb-5 border-b border-[#F0EDE8]">
        <h3 className="text-[9px] font-bold text-[#ADADAD] uppercase tracking-[0.25em] mb-3">
          {language === 'hi' ? 'उम्र / साइज़' : 'Age / Size'}
        </h3>
        <div className="grid grid-cols-3 gap-1.5">
          {ageSizes.map((size) => (
            <button key={size}
              onClick={() => setSelectedSizes(prev => prev.includes(size) ? prev.filter(s => s !== size) : [...prev, size])}
              className={`py-1.5 rounded-lg text-[10px] font-medium border transition-all ${selectedSizes.includes(size) ? 'bg-[#1A1A1A] text-white border-[#1A1A1A] shadow-sm' : 'bg-white text-[#6B6B6B] border-[#E0DBD4] hover:border-[#1A1A1A] hover:text-[#1A1A1A]'}`}>
              {size}
            </button>
          ))}
        </div>
  ...
  return (
    <div className="min-h-screen bg-[#F8F6F4]">
      ...
      <div className="relative w-full h-[360px] sm:h-[460px] md:h-[540px] overflow-hidden">
        ...
        />
        {/* Sky-blue colour grade for kids */}
        <div className="absolute inset-0 bg-sky-950/12" />
        ...
        <div className="absolute inset-0 flex flex-col justify-end pb-16 px-12 sm:px-16 lg:px-24">
          <nav className="flex items-center gap-2 text-[11px] text-white/70 mb-4 tracking-wide" style={{ textShadow: '0 1px 6px rgba(0,0,0,0.8)' }}>
            ...
            {activeCategory && (
              <>
                <span>/</span>
                <span className="text-white/90">{activeCategory} Years</span>
              </>
            )}
          </nav>
          <h1
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-8" onClick={() => setQuickViewProduct(null)}>
          ...
          <div className="relative bg-white rounded-2xl shadow-2xl w-full max-w-[800px] max-h-[90vh] overflow-hidden flex flex-col sm:flex-row" onClick={(e) => e.stopPropagation()}>
            ...
            <div className="flex-1 p-7 sm:p-9 overflow-y-auto flex flex-col">
              <p className="text-[9.5px] tracking-[0.25em] uppercase text-[#ADADAD] font-semibold mb-2">Fashion Points · Kids</p>
              ...
              {quickViewProduct.sizes && quickViewProduct.sizes.length > 0 && (
                <div className="mb-6">
                  <p className="text-[10px] font-semibold uppercase tracking-[0.2em] text-[#ADADAD] mb-3">Select Age / Size</p>
                  <div className="flex flex-wrap gap-2">
