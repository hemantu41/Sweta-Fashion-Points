...
const I = {
  ...
  truck:    'M1 3h15v13H1z M16 8h4l3 3v5h-7V8z M5.5 19.5a2 2 0 100-4 2 2 0 000 4z M18.5 19.5a2 2 0 100-4 2 2 0 000 4z',
...
export default function ProductDetailPage({ params }: { params: { id: string } }) {
  ...
  const discount      = (product?.originalPrice ?? 0) > product?.price!
  ...
  // XS is shown as OOS for demo when it appears in the sizes list
  const OOS = new Set(['XS']);
    ...
    <div style={{ background: C.cream, minHeight: '100vh', fontFamily: 'var(--font-lato, system-ui, sans-serif)' }}>
      ...
      <nav style={{ background: '#fff', borderBottom: `0.5px solid ${C.border}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: '10px 16px' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, flexWrap: 'wrap', fontSize: 12, color: C.muted }}>
            ...
            <span
              style={{ color: C.maroon, fontWeight: 500, maxWidth: 220, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}
            >
              {product.name}
            </span>
          </div>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, flexShrink: 0 }}>
                {thumbs.map((img, i) => (
                  <button
                    ...
                    style={{
                      width: 64, height: 74, borderRadius: 8, overflow: 'hidden', padding: 0,
                      cursor: 'pointer', flexShrink: 0, background: C.cream,
                      border: i === activeThumb
                        ? `1.5px solid ${C.maroon}`
      ...
      <div style={{ background: '#fff', borderTop: `0.5px solid ${C.border}` }}>
        <div style={{ maxWidth: 1200, margin: '0 auto', padding: 24 }}>
          <div className="grid grid-cols-1 md:grid-cols-[1fr_1px_1fr]" style={{ gap: 0 }}>
