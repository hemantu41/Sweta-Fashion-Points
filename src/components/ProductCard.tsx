...
export default function ProductCard({ product }: ProductCardProps) {
  ...
  return (
    <Link href={`/product/${product.productId || product.id}`} className="bg-white rounded-xl overflow-hidden card-hover group border border-[#E8E2D9] block">
      {/* Image Container */}
        ...
        {product.seller && (
          <div className="mt-2 pt-2 border-t border-[#F0EDE8]">
            <p className="text-xs text-[#6B6B6B] flex items-center gap-1">
              ...
              {product.seller.city && (
                <span className="text-[#999]">• {product.seller.city}</span>
              )}
            </p>
          </div>
        )}
