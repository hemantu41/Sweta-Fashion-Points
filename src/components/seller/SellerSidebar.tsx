...
const ICONS = {
  ...
  reviews:    'M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z',
  ...
  support:    'M18.364 5.636a9 9 0 010 12.728M15.536 8.464a5 5 0 010 7.072M12 12h.01M6.343 17.657a9 9 0 010-12.728M9.172 14.828a5 5 0 010-7.072',
  chevron:    'M9 18l6-6-6-6',
};

const PRODUCT_PATHS = ['/seller/dashboard/products', '/seller/dashboard/qc', '/seller/dashboard/add'];
...
export default function SellerSidebar({ sellerName, healthScore, orderBadge, productBadge, qcBadge, isOpen, onClose }: SellerSidebarProps) {
  ...
  // Auto-expand Products group when any child path is active
  const [productsOpen, setProductsOpen] = useState(() =>
    PRODUCT_PATHS.some(p => pathname === p || pathname.startsWith(p + '/'))
  );

  // Keep expanded when navigating to a child
  useEffect(() => {
    if (PRODUCT_PATHS.some(p => pathname === p || pathname.startsWith(p + '/'))) {
  ...
  const navItems: NavItem[] = [
    ...
    { label: 'Inventory',    href: '/seller/dashboard/inventory', icon: <Icon d={ICONS.inventory} /> },
    ...
    { label: 'Support',      href: '/seller/dashboard/support',   icon: <Icon d={ICONS.support} /> },
  ...
  function isActive(href: string) {
    return pathname === href || (href !== '/seller/dashboard' && pathname.startsWith(href + '/'));
  }
        ...
        <div className="flex items-center gap-3">
          ...
          <div className="min-w-0">
            ...
            <span className="inline-flex items-center gap-1 text-[10px] px-1.5 py-0.5 rounded-full bg-[#C49A3C]/20 text-[#DDB868] font-medium mt-0.5">
              <span className="w-1.5 h-1.5 rounded-full bg-[#C49A3C] inline-block" />
              Approved Seller
            </span>
          </div>
        </div>
        ...
        {navItems.map((item) => {
            ...
            return (
              <div key={item.label}>
                ...
                >
                  ...
                  <span className={groupActive ? 'text-white' : 'text-white/60 group-hover:text-white/90'}>{item.icon}</span>
                  <span className="flex-1 text-left">{item.label}</span>
                  {totalBadge > 0 && (
                    <span className="text-[10px] font-bold px-1.5 py-0.5 rounded-full bg-[#C49A3C]/25 text-[#DDB868] min-w-[18px] text-center mr-1">
                  ...
                  <svg
                    ...
                    className={`transition-transform duration-200 text-white/40 ${open ? 'rotate-90' : ''}`}
                  >
                    ...
                    {item.children.map((child) => {
                      ...
                      return (
                        <Link
                          ...
                          href={child.href}
                          onClick={onClose}
                          className={`flex items-center gap-2.5 px-3 py-2 rounded-lg text-xs transition-all duration-150 group relative ${
                            childActive
                              ? 'bg-[#C49A3C]/15 text-white font-semibold'
                              : 'text-white/55 hover:bg-white/8 hover:text-white'
                          }`}
                        >
                          {childActive && (
                            <span className="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-[#C49A3C] rounded-r-full" />
                          )}
                          <span className={childActive ? 'text-[#DDB868]' : 'text-white/50 group-hover:text-white/80'}>{child.icon}</span>
          ...
          return (
            ...
            >
              ...
              <span className={active ? 'text-white' : 'text-white/60 group-hover:text-white/90'}>{item.icon}</span>
